...
loaded_parameters = load_parameters('1DCNN_M3_parameters.pkl')

def mynetwork(x, parameters, isTraining, momentums = 0.9): 
    x = tf.reshape(x, [-1, 1, 1, 85], name = "x")   
...
x = tf.compat.v1.placeholder(tf.float32, shape=[None, 85], name="input_placeholder")
# tf.compat.v1.placeholder(tf.float32, [None, n_x], name = "x")  
scipy.io.whosmat('m3data.mat')

Cuprite_data = scipy.io.loadmat('M3_data.mat')
Cuprite_data =Cuprite_data['Data'] 
# print(Cuprite_data.shape)
# Cuprite_data= Cuprite_data[500:1500][:][:]
...
output_2d = np.reshape(output_, (1000, 304))
...
plt.title("M3(Ch-01) Data Mineral Classification Using 1-DCNN")
plt.show()
